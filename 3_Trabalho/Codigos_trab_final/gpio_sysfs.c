...
int setGPIO_In(int pin, char* modo)
{	
	/* This is a function to export a GPIO pin - and set it's direction to output... */
	int valid_pins[]={VALID_PINS};
	int c;
	...
	/* ...and then we'll write "out" to the direction file.*/
	if (fwrite("in", sizeof(char), 4, sysfs_handle) != 4)
	{
	
		fprintf(stderr, "ERROR: Unable to read direction for GPIO%d\n", pin);
		return 4;
	}

	/* we'll open the edge file*/
	char str_edge_file[MAXSTR];
	snprintf(str_edge_file,(MAXSTR*sizeof(char)),"/sys/class/gpio/gpio%d/edge",pin);
	if((sysfs_handle = fopen(str_edge_file,"w")) == NULL)
	{
		fprintf(stderr, "ERROR:  C'est pas possible d'ouvrir le fichier edge... \n(ce programme s'exécute-t-il comme root?)\n");
		return 5;
	}

	/*we'll write de reading mode on edge file*/
	if (fwrite(modo, sizeof(char), 8, sysfs_handle) != 8)
	{
		fprintf(stderr, "ERROR: Unable to read edge for GPIO%d\n", pin);
		return 6;
	}
...
int GPIO_Read(int pin)
{
	...
	struct pollfd pfd;//Para a interrupção
	...
	if ((sysfs_handle = fopen(str_value_file, "r")) == NULL)//Mudei para leitura
	{
		fprintf(stderr, "ERROR: Cannot open value file for pin %d...\n(Has the pin been exported?)\n", pin);
		return 2;
	}

	pfd.fd = open(str_value_file, O_RDONLY);

	/* If the file is good - then we write the string value "0" or "1" to it. */
	char str_val[2];
	//snprintf (str_val, (2*sizeof(char)), "%d", value);

	/*if((fread(str_val, sizeof(char), 2,sysfs_handle)) != 2)// Alterei para ler o valor
	{
		fprintf(stderr, "ERROR: Cannot read  GPIO pin %d\n", pin);
		return 2;
	}*/
	fclose(sysfs_handle);
	
	read(pfd.fd, str_val, 1);
	...
	{	
		...
		printf("ERRO: Problema na leitura da porta %d\n", pin);
		return 3;//Erro
	close(pfd.fd);
}
